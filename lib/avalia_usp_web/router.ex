defmodule AvaliaUspWeb.Router do
  use AvaliaUspWeb, :router

  use AshAuthentication.Phoenix.Router

  import AshAuthentication.Plug.Helpers

  pipeline :graphql do
    plug AshGraphql.Plug
  end

  pipeline :browser do
    plug :accepts, ["html"]
    plug :fetch_session
    plug :fetch_live_flash
    plug :put_root_layout, html: {AvaliaUspWeb.Layouts, :root}
    plug :protect_from_forgery
    plug :put_secure_browser_headers
    plug :load_from_session
  end

  pipeline :api do
    plug :accepts, ["json"]
    plug :load_from_bearer
    plug :set_actor, :user
  end

  scope "/gql" do
    pipe_through [:graphql]

    forward "/playground", Absinthe.Plug.GraphiQL,
      schema: Module.concat(["AvaliaUspWeb.GraphqlSchema"]),
      socket: Module.concat(["AvaliaUspWeb.GraphqlSocket"]),
      interface: :simple

    forward "/", Absinthe.Plug, schema: Module.concat(["AvaliaUspWeb.GraphqlSchema"])
  end

  scope "/api/json" do
    pipe_through [:api]

    forward "/swaggerui", OpenApiSpex.Plug.SwaggerUI,
      path: "/api/json/open_api",
      default_model_expand_depth: 4

    forward "/", AvaliaUspWeb.AshJsonApiRouter
  end

  scope "/", AvaliaUspWeb do
    pipe_through :browser

    ash_authentication_live_session :authenticated_routes do
      # in each liveview, add one of the following at the top of the module:
      #
      # If an authenticated user must be present:
      # on_mount {AvaliaUspWeb.LiveUserAuth, :live_user_required}
      #
      # If an authenticated user *may* be present:
      # on_mount {AvaliaUspWeb.LiveUserAuth, :live_user_optional}
      #
      # If an authenticated user must *not* be present:
      # on_mount {AvaliaUspWeb.LiveUserAuth, :live_no_user}
    end
  end

  scope "/", AvaliaUspWeb do
    pipe_through [:browser]

    live_session :default,
      on_mount: {AvaliaUspWeb.LiveUserAuth, :current_user} do
      live "/", HomeLive, :index

      live "/disciplinas", DisciplinasLive.Index, :index

      live "/professores/ranking", RankingLive

      live "/professores/:professor_nome", ProfessoresLive.Show, :show

      live "/professores/:professor_nome/disciplinas/:disciplina_nome",
           DisciiplinasLive.Show,
           :show

      live "/avaliar/:professor_nome", AvaliarLive, :index

      live "/solicitacoes/:professor_nome/nova-disciplina",
           SolicitacoesLive.Index,
           :index

      live "/professores/:professor_nome/avaliacoes/:avaliacao_id",
           AvaliacoesLive.Show,
           :show
    end
  end

  scope "/", AvaliaUspWeb do
    pipe_through :browser

    auth_routes AuthController, AvaliaUsp.Accounts.User, path: "/auth"
    sign_out_route AuthController

    # Remove these if you'd like to use your own authentication views
    sign_in_route register_path: "/register",
                  reset_path: "/reset",
                  auth_routes_prefix: "/auth",
                  on_mount: [{AvaliaUspWeb.LiveUserAuth, :live_no_user}],
                  overrides: [
                    AvaliaUspWeb.AuthOverrides,
                    Elixir.AshAuthentication.Phoenix.Overrides.DaisyUI
                  ]
  end

  # Other scopes may use custom stacks.
  # scope "/api", AvaliaUspWeb do
  #   pipe_through :api
  # end

  # Enable LiveDashboard and Swoosh mailbox preview in development
  if Application.compile_env(:avalia_usp, :dev_routes) do
    # If you want to use the LiveDashboard in production, you should put
    # it behind authentication and allow only admins to access it.
    # If your application does not have an admins-only section yet,
    # you can use Plug.BasicAuth to set up some basic authentication
    # as long as you are also using SSL (which you should anyway).
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through :browser

      live_dashboard "/dashboard", metrics: AvaliaUspWeb.Telemetry
      forward "/mailbox", Plug.Swoosh.MailboxPreview
    end
  end

  if Application.compile_env(:avalia_usp, :dev_routes) do
    import AshAdmin.Router

    scope "/admin" do
      pipe_through :browser

      ash_admin "/"
    end
  end
end
